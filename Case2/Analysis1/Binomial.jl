...
# In this script, we generate the result for Case 2a
...
Pmin=0.1 # lower bound for the profile  
...
spl=Spline1D(Prange,nllP.-maximum(nllP),w=ones(length(Prange)),k=3,bc="nearest",s=1.8)
...
s1b2 = annotate!(-100*0.6, -7, text(L"C_{0.95}^{\mathrm{o}} - \hat{C} ", :left, tfs,rotation=90))
...
kmax=0.04# upper bound for the profile 
...
spl=Spline1D(krange,nllk.-maximum(nllk),w=ones(length(krange)),k=3,bc="nearest",s=0.1)
yyk=evaluate(spl,krange)
s2a=plot(krange,yyk,lw=2,xlim=(0,0.04),ylim=(-3,0.1),legend=false,tickfontsize=tfs,labelfontsize=tfs,grid=false,
xticks=([0,0.01,0.02,0.03,0.04], [latexstring("0"),"", latexstring("0.02"),"",latexstring("0.04")]), yticks=([-3,-2,-1,0],[latexstring("-3"),latexstring("-2"),latexstring("-1"),latexstring("0")]),framestyle=:box,left_margin = 15mm)
s2a=hline!([llstar],lw=2)
s2a=vline!([kmle],lw=2)
s2a = annotate!(0.02, -4, text(L"λ", :left, tfs))
s2a = annotate!(-0.011, -1.5, text(L"\bar{\ell}_{p}", :left, tfs,rotation=90))
s2a = annotate!(-0.016, 0.41, text(L"(\mathrm{d})", :left, tfs))
...
s2b2 = annotate!(-100*0.6, -7, text(L"C_{0.95}^{\mathrm{o}} - \hat{C} ", :left, tfs,rotation=90))
...
llstar = -quantile(Chisq(df),0.95)/2 # 95% asymptotic threshold
nptss=40 # points on the profile
c0min=0.001 # lower bound for the profile  
c0max=0.99# upper bound for the profile 
...
# Plot the univariate profile likelihood for k.
spl=Spline1D(c0range,nllc0.-maximum(nllc0),w=ones(length(c0range)),k=2,bc="nearest",s=0.089)
...
s3b2 = annotate!(-100*0.6, -7, text(L"C_{0.95}^{\mathrm{o}} - \hat{C} ", :left, tfs,rotation=90))
