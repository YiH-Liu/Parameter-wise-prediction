...
using FilePathsBase
using LaTeXStrings
# In this script, we create Supplementary Information (SI) Figure S4
...
#Read the CSV file: coordinate for subpopulation 1 and 2 at time t
...
s2 = scatter!((index2.i.-1).*Δ, (index2.j.-1).*Δ, label=false, marker=:circle,
            ...
            grid=false,tickfontsize = tfs,aspect_ratio = :equal,
            color=:green,markerstrokecolor=:green,framestyle=:box,alpha=0.6)
s2 = annotate!(W*0.03, H*1.4, text(L"k = 200", :left, tfs))
...
d1a = scatter!(1:1:200,C_1_0, label=false, marker=:circle,
        xlims = [1,200],ylims = [-10,100],xticks=([1,100,200], [latexstring("1"),"",latexstring("200")]),
        yticks=([0,20,40,60,80,100],[latexstring("0"),"","","","",latexstring("100")]),legend = false,markersize=1.5,
        grid=false,tickfontsize = tfs,aspect_ratio = :equal,
        color=:red,markerstrokecolor=:red,framestyle=:box)
d1a = annotate!(200*0.03, 94*1.15, text(L"k = 0", :left, tfs))
d1a = annotate!(200*0.5, -21*1.1, text(L"i", :left, tfs))
d1a = annotate!(-200*0.15, 45, text(L"C_{1}^{\mathrm{o}}", :left, tfs,rotation=90))
d1a = annotate!(-W*0.25, 94*1.15, text(L"(\mathrm{c})", :left, tfs))
...
d1b = annotate!(200*0.03, 94*1.15, text(L"k = 0", :left, tfs))
d1b = annotate!(200*0.5, -21*1.1, text(L"i", :left, tfs))
d1b = annotate!(-200*0.15, 45, text(L"C_{2}^{\mathrm{o}}", :left, tfs,rotation=90))
d1b = annotate!(-W*0.25, 94*1.15, text(L"(\mathrm{d})", :left, tfs))
...
d2a = scatter!(1:δ:200,C_1[1:δ:200], label=false, marker=:circle,
        xlims = [1,200],ylims = [-10,100],xticks=([1,100,200], [latexstring("1"),"",latexstring("200")]),
        yticks=([0,20,40,60,80,100],[latexstring("0"),"","","","",latexstring("100")]),legend = false,markersize=ms,
        grid=false,tickfontsize = tfs,aspect_ratio = :equal,
        color=:red,markerstrokecolor=:red,framestyle=:box)
d2a = annotate!(200*0.03, 94*1.15, text(L"k = 200", :left, tfs))
d2a = annotate!(200*0.5, -21*1.1, text(L"i", :left, tfs))
d2a = annotate!(-200*0.15, 45, text(L"C_{1}^{\mathrm{o}}", :left, tfs,rotation=90))
d2a = annotate!(-W*0.25, 94*1.15, text(L"(\mathrm{e})", :left, tfs))
...
d2b = scatter!(1:δ:200,C_2[1:δ:200], label=false, marker=:circle,
        ...
        grid=false,tickfontsize = tfs,aspect_ratio = :equal,
        color=:green,markerstrokecolor=:green,framestyle=:box)
d2b = annotate!(200*0.03, 94*1.15, text(L"k = 200", :left, tfs))
d2b = annotate!(200*0.5, -21*1.1, text(L"i", :left, tfs))
d2b = annotate!(-200*0.15, 45, text(L"C_{2}^{\mathrm{o}}", :left, tfs,rotation=90))
d2b = annotate!(-W*0.25, 94*1.15, text(L"(\mathrm{f})", :left, tfs))
...
d3a = scatter!(1:δ:200,C_1[1:δ:200], label=false, marker=:circle,
        ...
        grid=false,tickfontsize = tfs,aspect_ratio = :equal,
        color=:red,markerstrokecolor=:red,framestyle=:box)
d3a = annotate!(200*0.03, 94*1.15, text(L"k = 200", :left, tfs))
d3a = annotate!(200*0.5, -21*1.1, text(L"i", :left, tfs))
d3a = annotate!(-200*0.15, 45, text(L"C_{1}^{\mathrm{o}}", :left, tfs,rotation=90))
d3a = annotate!(-W*0.25, 94*1.15, text(L"(\mathrm{g})", :left, tfs))
...
d3b = scatter!(1:δ:200,C_2[1:δ:200], label=false, marker=:circle,
        ...
        grid=false,tickfontsize = tfs,aspect_ratio = :equal,
        color=:green,markerstrokecolor=:green,framestyle=:box)
d3b = annotate!(200*0.03, 94*1.15, text(L"k = 200", :left, tfs))
d3b = annotate!(200*0.5, -21*1.1, text(L"i", :left, tfs))
d3b = annotate!(-200*0.15, 45, text(L"C_{2}^{\mathrm{o}}", :left, tfs,rotation=90))
d3b = annotate!(-W*0.25, 94*1.15, text(L"(\mathrm{h})", :left, tfs))

f = plot(s1, s2, d1a, d1b, d2a, d2b,d3a,d3b,layout = layout = @layout[a{0.054h} b{0.054h};c{0.31h} d{0.31h}; e{0.31h} f{0.31h};g{0.31h} h{0.31h}], link = :x, 
top_margin = 1.5mm, bottom_margin=1mm, right_margin = 0mm,left_margin = 0mm,size=(580,550))  # Here we set the top margin
