...
# In this script, we generate the data required to create Figure 2 and Supplementary Information (SI) Figure S1.
...
# Simulate averaged count data, averaged over M realisation
M = 100
I = Int(W/Δ + 1); J = Int(H/Δ + 1); 
global A1 = zeros(J,I); global A2 = zeros(J,I)
for i = 1:M
    # Simulate the discrete model at time t.
    local A1_0,A2_0,N1_0,N2_0 = init(W,H,Δ,h_0,d1_0,d2_0)
    @time local A1t,A2t = discrete_simulation(A1_0,A2_0,N1_0,N2_0,P_m,ρ,Rg,Rr,W,H,Δ,τ,t)
    global A1 = A1.+A1t; global A2 = A2.+A2t;
end

A1 = A1./M; A2 = A2./M;
# Averaged count data and save it
count1 = sum(A1, dims=1)
count2 = sum(A2, dims=1)
countall = DataFrame(a = vec(count1),b = vec(count2))
CSV.write("$path\\Adata.csv", countall)
